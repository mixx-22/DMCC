...
const Organizations = ({ schedule = {}, setFormData = () => {} }) => {
  ...
  useEffect(() => {
    const fetchStandardClauses = async () => {
      ...
      if (
        ...
        standard.clauses &&
        Array.isArray(standard.clauses)
      ) {
      ...
      const standardId =
        typeof standard === "string" ? standard : standard.id || standard._id;
  ...
  const isInternalAuditor = useMemo(() => {
    ...
    return normalizedRoles.some((role) => {
      ...
      return typeof title === "string"
        ? title.toLowerCase().includes("internal auditor")
        : false;
    });
  }, [user]);
  ...
  const visibleOrganizations = useMemo(() => {
    if (!isInternalAuditor || !currentUserId) {
      return organizations || [];
    }

    return (organizations || []).filter((org) => {
      const auditors = org?.auditors || [];
      return auditors.some((auditor) => {
        if (!auditor) return false;
        const auditorId =
          typeof auditor === "object"
            ? auditor._id || auditor.id || auditor.userId || auditor.employeeId
            : auditor;
        return String(auditorId) === String(currentUserId);
      });
    });
      ...
      <Flex justify="space-between" align="center">
        ...
        <Text fontSize="xs" color="gray.500">
          {visibleOrganizations?.length > 0
            ? `${visibleOrganizations?.length} Organization${visibleOrganizations?.length === 1 ? `` : `s`}`
            : `No Organizations Yet`}
        </Text>
      </Flex>

      {loading && visibleOrganizations?.length < 1 && (
