...
const AuditKpiDashboard = () => {
  ...
  const generateKpiReportHTML = () => {
    ...
    const author = currentUser?.firstName || currentUser?.name || "User";
    const scheduleTitle = selectedScheduleDisplay;

    const findingsByClauseRows = (kpiData.findingsPerClause || [])
      .map(
        (item) =>
          `<div class="row"><span class="label">${item.clause || "N/A"}</span><span class="value">${item.count || 0}</span></div>`,
      )
      .join("");
    ...
    const teamMetricsRows = (kpiData.ncMetricsPerTeam || [])
      .map(
        (team) =>
          `<div class="row"><span class="label">${team.team || "Unknown Team"}</span><span class="value">Findings: ${team.totalFindings || 0} · NC: ${team.totalNC || 0} · NC %: ${team.ncPercentage?.toFixed(1) || 0}% · Contribution: ${team.ncContributionPercentage?.toFixed(1) || 0}%</span></div>`,
      )
      .join("");

    const teamContributionRows = (kpiData.ncContributionPerTeam || [])
      .map(
        (team) =>
          `<div class="row"><span class="label">${team.team || "Unknown Team"}</span><span class="value">${team.contributionPercentage?.toFixed(1) || 0}% (${team.teamNC || team.ncCount || 0} NCs)</span></div>`,
      )
      .join("");
    ...
    return `
      ...
      <html>
        <head>
          ...
          <style>
            body { font-family: Arial, sans-serif; color: #000; padding: 24px; background: #fff; }
            .header { text-align: center; margin-bottom: 20px; }
            .title { font-size: 22px; font-weight: 700; margin-bottom: 8px; }
            .subtitle { font-size: 13px; color: #333; margin-bottom: 4px; }
            .section { margin-bottom: 18px; }
            .section-title { font-weight: 700; margin-bottom: 10px; text-decoration: underline; }
            .row { display: flex; justify-content: space-between; margin-bottom: 5px; }
            .label { font-weight: 600; width: 65%; }
        ...
        <body>
          <div class="header">
            <div class="title">Audit KPI Report</div>
            <div class="subtitle">Generated by: ${author}</div>
            <div class="subtitle">Generated on: ${reportDate}</div>
            <div class="subtitle">Year: ${selectedYearDisplay}</div>
            <div class="subtitle">Schedule: ${scheduleTitle}</div>
          </div>

          <div class="section">
            <div class="section-title">Performance Metrics</div>
            <div class="row"><span class="label">Audit Completion Rate</span><span class="value">${kpiData.auditCompletionRate?.toFixed(1) || 0}%</span></div>
            <div class="row"><span class="label">Audit Execution Rate</span><span class="value">${kpiData.auditExecutionRate?.toFixed(1) || 0}%</span></div>
            <div class="row"><span class="label">Average Audit Duration</span><span class="value">${kpiData.averageAuditDuration?.toFixed(1) || 0} days</span></div>
            <div class="row"><span class="label">Total Organizations</span><span class="value">${kpiData.totalOrganizations || 0}</span></div>
            <div class="row"><span class="label">Total Findings</span><span class="value">${kpiData.totalFindings || 0}</span></div>
          </div>

          <div class="section">
            <div class="section-title">Non-Conformity Analysis</div>
            <div class="row"><span class="label">Non-Conformity Rate</span><span class="value">${kpiData.nonConformityRate?.toFixed(1) || 0}%</span></div>
            <div class="row"><span class="label">Major Non-Conformities</span><span class="value">${kpiData.majorVsMinorCount?.MAJOR_NC || 0}</span></div>
            <div class="row"><span class="label">Minor Non-Conformities</span><span class="value">${kpiData.majorVsMinorCount?.MINOR_NC || 0}</span></div>
            <div class="row"><span class="label">Observations</span><span class="value">${kpiData.majorVsMinorCount?.OBSERVATIONS || 0}</span></div>
            <div class="row"><span class="label">Corrective Action Closure Rate</span><span class="value">${kpiData.correctiveActionClosureRate?.toFixed(1) || 0}%</span></div>
          </div>

          <div class="section">
            <div class="section-title">Formulas</div>
            <pre>Audit Completion Rate = (Audits Completed / Audits Planned) × 100
Audit Execution Rate = (Audits Executed / Audits Scheduled) × 100
Average Audit Duration = Total Audit Days / Number of Audits
Non-Conformity Rate = (NC Findings / Total Findings) × 100
Corrective Action Closure Rate = (Closed Corrective Actions / Total Corrective Actions) × 100</pre>
          </div>

          <div class="section">
            <div class="section-title">Findings by ISO Clause</div>
            ${findingsByClauseRows || "<div>No findings recorded for selected schedules</div>"}
          </div>

          <div class="section">
            <div class="section-title">Team Performance Metrics</div>
            ${teamMetricsRows || "<div>No team performance data available</div>"}
          </div>

          <div class="section">
            <div class="section-title">Team Contribution Analysis</div>
            <div class="row"><span class="label">Overall NC Percentage</span><span class="value">${kpiData.overallNcPercentage?.toFixed(1) || 0}%</span></div>
            ${teamContributionRows || "<div>No team contribution data available</div>"}
          </div>
        </body>
      </html>
    `;
